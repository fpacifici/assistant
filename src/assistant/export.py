...
class ExportedDocumentMetadata(TypedDict):
    """Serialized representation of a document metadata entry."""
...
class ExportedData(TypedDict):
    ...
    document_metadata: list[ExportedDocumentMetadata]
...
def _export_application_data(
    session: Session,
) -> tuple[list[ExportedExternalSource], list[ExportedDocument], list[ExportedDocumentMetadata]]:
    ...
    Returns:
        A tuple containing the exported external sources, documents, and document metadata.
    """
    ...
    for document in session.scalars(select(Document)):
        ...
        if isinstance(document_format, DocumentFormat):
            format_value = document_format.value
        else:
            format_value = str(document_format)
    ...
    document_metadata: list[ExportedDocumentMetadata] = []
    for metadata in session.scalars(select(DocumentMetadata)):
        document_metadata.append(
            {
                "document_uuid": str(metadata.document_uuid),
                "key": metadata.key,
                "value": metadata.value,
            },
        )

    return external_sources, documents, document_metadata
...
def _write_logical_dump(engine: Engine, output_path: Path) -> None:
    ...
    with session_factory() as session:
        external_sources, documents, document_metadata = _export_application_data(session)
